...
var AppointmentPlanner;
(function (AppointmentPlanner) {
    $(function () {
        ...
        var hospitaldepartments = new ej.DropDownList($("#dept"), {
            width: "150px",
            dataSource: window["departmentName"],
...
function onWindowResize() {
    ...
    if ($("#searchdata").is(":visible"))
        $(".displayright").find(".e-togglebutton").click();
...
function onWaitingOpen(e) {
    ...
    var waitAppointment = new ej.DataManager(window["waitinglist"]).executeLocal(new ej.Query().where("Id", ej.FilterOperators.equal, wid));
...
function onHover() {
    $(".waitinglistli").mouseout(function (e) {
        ...
        var hoverObj = new ej.DataManager(window["waitinglist"]).executeLocal(new ej.Query().where("Id", ej.FilterOperators.equal, parseInt($(e.currentTarget).find("#waitid").html())));
        ...
        $("#docname").html(hoverName[0].Text + "  (" + hoverName[0].Designation + ")");
        $("#docstarttime").html(hoverObj[0].StartTime.toLocaleString());
        $("#docendtime").html(hoverObj[0].EndTime.toLocaleString());
    });
}
...
function onSchedule() {
    ...
    if (schObj.model.orientation == "vertical" && currentview == "month")
        $("#DoctorSchedule").find(".e-workcellstab td:first-child").css("border-left", "1px solid #c3c3c3");
    if (schObj.model.orientation == "vertical" && currentview != "month")
        $("#DoctorSchedule").find(".e-ampmdisplay").css("margin-right", "4px");
    $("#DoctorSchedule").find(".e-scheduleheader").parent().append($("<tr></tr>").append("<div id='grid2'></div>"));
}
...
function onAppointmentClick(args) {
    if ($(args.e.currentTarget).attr("id") == "appsubmit")
        save();
    else
        cancel();
}
...
function _doctorDesignation(type1) {
    var result = new ej.DataManager(window["categorylist"]).executeLocal(new ej.Query().where("Value", ej.FilterOperators.equal, type1))[0].Text;
    return result;
}
function _color(cat) {
...
function _height(start, end) {
    ...
    if (obj.model.currentView != "month") {
        var srtTime = new Date(new Date(rStartTime).getTime());
        var endTime = new Date(new Date(rEndTime).getTime());
        var calTime = endTime - srtTime;
        appHeight = ((parseFloat(calTime)) / (60 * 1000) * (cellHeight * obj.model.timeScale.minorSlotCount) / (obj.model.timeScale.majorSlot)) - 1;
    }
    else
...
function _getResources(id, type1) {
    var result;
    var schObj = $("#DoctorSchedule").ejSchedule('instance');
    if (type1 == "image") {
        var resObj = new ej.DataManager(window["doctorName"]).executeLocal(new ej.Query().where(schObj.model.resources[schObj.model.resources.length - 1].resourceSettings["id"], ej.FilterOperators.equal, id));
...
function onDoctorlistHide(args) {
    ...
    if (args.text == "All") {
        ...
        for (var a = 0; a < window["doctorName"].length; a++)
            doctorlistChecked.push(a);
    ...
    if (categorylistChecked.indexOf(0) == 0)
        categorylistChecked.splice(0, 1);
    ...
    if (args.item.parent().attr("id") == "categorylist" && categoryCheck) {
        ...
        if (args.text == "All") {
            ...
            if (checked.indexOf(0) == 0)
                checked.splice(0, 1);
            if (checked.length == window["categorylist"].length)
...
function filterAppointments(filterConditions, schObj) {
    var result, queryManagar = new ej.Query(), matchcaseVal;
...
function save() {
    ...
    for (var index = 0; index < formelement.length; index++) {
        ...
        if (columnName != undefined) {
            if (columnName == "")
                columnName = formelement[index].id;
            if (columnName != "" && obj[columnName] == null) {
                var value = formelement[index].value;
    ...
    $("#customWindow").ejDialog("close");
    var schObj = $("#DoctorSchedule").ejSchedule('instance');
    var predicate1 = new ej.Predicate(schObj._appointmentSettings["startTime"], ej.FilterOperators.lessThanOrEqual, new Date(obj.StartTime), true).and(schObj._appointmentSettings["endTime"], ej.FilterOperators.greaterThanOrEqual, new Date(new Date(obj.EndTime)), true);
    ...
    if (!window["addlist"] && !window["waitinglistedit"]) {
        ...
        if (!window["appointedit"] || window["drop"]) {
            index = window["waitinglist"].indexOf(new ej.DataManager(window["waitinglist"]).executeLocal(new ej.Query().where("Id", ej.FilterOperators.equal, clone.Id))[0]);
        ...
        if (window["waitinglistedit"]) {
            new ej.DataManager(window["waitinglist"]).remove("Id", obj.Id, "waitinglist");
        }
...
function onSearchResultSelect(args) {
    ...
    switch (srchObj.selectedTextValue) {
        case "ID":
            ...
            break;
        case "Name":
            if (!$.isNumeric(parseInt(args.model.value)))
                searchlist.push({ field: "Subject", operator: "contains", value: args.model.value });
            if (searchlist.length > 0)
                result = filterAppointments(searchlist, schObj);
            break;
        case "Description":
        ...
        case "Category":
            if (!$.isNumeric(parseInt(args.model.value)))
                searchlist.push({ field: "Category", operator: "contains", value: args.model.value });
            if (searchlist.length > 0)
                result = filterAppointments(searchlist, schObj);
            break;
        default:
