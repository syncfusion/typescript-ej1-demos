...
let monthNames = ["Jan", "Feb", "Mar", "Apl", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
let skyCondition = ["Snow", "Rain Storm", "Thunder", "Rainy", "Cloudy", "Partly Cloudy", "Sunny", "Partly Sunny"];
let rowDefModel = {
    rowHeight: 24,
    lineColor: "#A8A8A8",
    lineWidth: 0.5,
...
let hourGridData: any;
let avgGridData: any;
let avgChartData: any;
...
let weatherData: Array<weatherDataClass>;

let avgChartValuesObj = {
...
module WeatherAnalysis {
    $(function () {
        ...
        (<any>window).WeatherDataObject = weatherData;
        let datamanager1 = new ej.DataManager((<any>window).WeatherDataObject).executeLocal(new ej.Query());

        let hourGrid = new ej.Grid($("#HourGrid"), {
        ...
        let hourChartobj = new ej.datavisualization.Chart($("#HourChart"), {
            isResponsive: false,
           rowDefinitions: [{
                        rowHeight: 24,
                        lineColor: "#A8A8A8",
            ...
            primaryXAxis: {
                range: { min: (<any>window).WeatherDataObject[0].Date, max: (<any>window).WeatherDataObject[(<any>window).WeatherDataObject.length - 1].Date, interval: 6 },
                intervalType: 'Hours',
        ...
        (<any>window).AverageData = AverageData;
        ...
        var averageData: any = new ej.DataManager((<any>window).AverageData).executeLocal(new ej.Query());
        let columnNames: any[];
        columnNames = [];
...
function weatherSelection(target: any) {
    switch (target.id) {
        case "ChartHour":
...
function hourChartLoad(sender: any) {
...
function getHourData() {
    ...
    var series3: any = [];
    var series4: any = [];
    $.each((<any>window).WeatherDataObject, function (index, value) {
...
//It is used to set data source for the chart to series in model
function averageChartLoad(sender: any) {
...
function getAverageData() {
    var series1: any = [];
    var series2: any = [];
    $.each((<any>window).AverageData[0], function (index, value) {
        if (index != "Average") {
            var point1 = { XValue: index, YValue: value };
            series1.push(point1);
        }
    });
    $.each((<any>window).AverageData[1], function (index, value) {
        if (index != "Average") {
            var point1 = { XValue: index, YValue: value };
            series2.push(point1);
        }
...
function weatherDataValues(i: number) {
